...
# -------------------------
def publish_markers(output_dicts, pub_2D_detected_objects, pub_3D_detected_objects, pub_center_detected_objects, pub_detected_class, class_names):
    rospy.loginfo("📡 publishing /detected_2D_Box..")

    markers_2D  = MarkerArray()
    markers_3D = MarkerArray()
    markers_center = MarkerArray()
    ...
    for i, output in enumerate(output_dicts):
        for j, (box, label, score) in enumerate(zip(output["pred_boxes"], output["pred_labels"], output["pred_scores"])):
            ...
            # 1. Create 2D Box marker             
            marker_2D = Marker()
            marker_2D.header = Header()
            marker_2D.header.stamp = rospy.Time.now()
            marker_2D.header.frame_id = "velodyne"
            marker_2D.ns = "detected_2D_Box"
            marker_2D.id = i * 1000 + j
            marker_2D.type = Marker.CUBE
            marker_2D.action = Marker.ADD
            marker_2D.pose.position.x = x_center
            marker_2D.pose.position.y = y_center
            marker_2D.pose.position.z = 0.0  # For 2D, force z to zero
            marker_2D.scale.x = x_length
            marker_2D.scale.y = y_length
            marker_2D.scale.z = 0.0  # 2D box, z-scale to 0
            
            marker_2D.pose.orientation.x = 0.0
            marker_2D.pose.orientation.y = 0.0
            marker_2D.pose.orientation.z = qz
            marker_2D.pose.orientation.w = qw
            
            marker_2D.color.a = 0.5
            marker_2D.color.r = color[0]
            marker_2D.color.g = color[1]
            marker_2D.color.b = color[2]
            marker_2D.lifetime = rospy.Duration(0.1)
            markers_2D.markers.append(marker_2D)


            # 2. Create 3D Box marker 
            marker_3D = Marker()
            marker_3D.header = Header()
            marker_3D.header.stamp = rospy.Time.now()
            marker_3D.header.frame_id = "velodyne"
            marker_3D.ns = "detected_3D_Box"
            marker_3D.id = i * 1000 + j
            marker_3D.type = Marker.CUBE
            marker_3D.action = Marker.ADD
            marker_3D.pose.position.x = x_center
            marker_3D.pose.position.y = y_center
            marker_3D.pose.position.z = z_center
            marker_3D.scale.x = x_length
            marker_3D.scale.y = y_length
            marker_3D.scale.z = z_length
            
            marker_3D.pose.orientation.x = 0.0
            marker_3D.pose.orientation.y = 0.0
            marker_3D.pose.orientation.z = qz
            marker_3D.pose.orientation.w = qw            
            
            marker_3D.color.a = 0.5
            marker_3D.color.r = color[0]
            marker_3D.color.g = color[1]
            marker_3D.color.b = color[2]
            marker_3D.lifetime = rospy.Duration(0.2)
            markers_3D.markers.append(marker_3D)


            # 3. Create center marker 
            marker_center = Marker()
            marker_center.header = Header()
            marker_center.header.stamp = rospy.Time.now()
            marker_center.header.frame_id = "velodyne"
            marker_center.ns = "detected_center"
            marker_center.id = i * 1000 + j
            marker_center.type = Marker.SPHERE
            marker_center.action = Marker.ADD
            marker_center.pose.position.x = x_center
            marker_center.pose.position.y = y_center
            marker_center.pose.position.z = z_center
            ...
            marker_center.scale.x = 0.3
            marker_center.scale.y = 0.3
            marker_center.scale.z = 0.3
            marker_center.color.a = 1.0
            marker_center.color.r = 0.0
            marker_center.color.g = 1.0
            marker_center.color.b = 0.0             
            marker_center.lifetime = rospy.Duration(0.2)
            markers_center.markers.append(marker_center)


            # 4. Create txt marker 
            marker_text = Marker()
            marker_text.header = Header(stamp=rospy.Time.now(), frame_id="velodyne")
            marker_text.ns     = "detected_class"
            marker_text.id     = i * 1000 + j
            marker_text.type   = Marker.TEXT_VIEW_FACING
            marker_text.action = Marker.ADD
            marker_text.pose.position.x = x_center
            marker_text.pose.position.y = y_center
            marker_text.pose.position.z = z_center + z_length / 2 + 0.2
            marker_text.text    = f"{class_name}: {score:.2f}"
            marker_text.scale.z = 0.2
            marker_text.color.r = 1.0
            marker_text.color.g = 1.0
            marker_text.color.b = 1.0
            marker_text.color.a = 1.0
            marker_text.lifetime = rospy.Duration(0.2)
            markers_text.markers.append(marker_text)        

    # publish topics
    pub_2D_detected_objects.publish(markers_2D)
    pub_3D_detected_objects.publish(markers_3D)
    pub_center_detected_objects.publish(markers_center)
    pub_detected_class.publish(markers_text)
...
def lidar_callback(msg, args):
    ...
    voxelnext_model, lidar_dataset, pub_2D_detected_objects, pub_3D_detected_objects, pub_center_detected_objects, pub_detected_class  = args
    ...
    try:
        ...
        publish_markers(output_dicts, pub_2D_detected_objects, pub_3D_detected_objects, pub_center_detected_objects, pub_detected_class, voxelnext_model.class_names)
    except Exception as e:
...
def main():
    ...
    rospy.init_node('VoxelNeXt_2D_3D_center_object_detect', anonymous=False)
    ...
    rospy.loginfo("✅ Publishers for /detected_2D_Box created")
    
    pub_3D_detected_objects = rospy.Publisher('/detected_3D_Box', MarkerArray, queue_size=10)
    ...
    rospy.loginfo("✅ Publishers for /detected_center created")
    ...
    rospy.Subscriber(
        ...
        PointCloud2,
        lidar_callback,
        callback_args=(voxelnext_model, lidar_dataset, pub_2D_detected_objects, pub_3D_detected_objects, pub_center_detected_objects, pub_detected_class),
        queue_size=1,
        buff_size=2**24
    )
